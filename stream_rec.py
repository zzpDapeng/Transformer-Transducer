...
class stream_rec:
    ...
    def __generate_dictionary(self):
        ...
        with open('data/aishell/grapheme_table.txt', 'r') as f:
            lines = f.readlines()
        for line in lines:
            index = str(lines).split(' ')[1]
            index = int(index)
            word = str(lines).split(' ')[0]
    ...
    def reset_parameter(self):
        ...
        :return:
        """
        self.audio_data = np.empty((0, 1), dtype=np.short)
        self.frame_num = 0
        self.win_position = 0
        self.result = []
    ...
    def __init_view(self):
        ...
        self.font = font.Font(family='song ti', size=18)
        ...
        self.time_label = tk.Label(self.window,
                                   text='Record time:',
                                   font=('Arial', 12))
        self.time_label.place(x=20, y=450, anchor='nw')
        ...
        self.time_entry = tk.Entry(self.window,
                                   ...
                                   font=('Arial', 12),
                                   bg='white')
        self.time_entry.place(x=130, y=450, anchor='nw')

        # 录制时间单位标签
        self.second_label = tk.Label(self.window,
                                     text='s',
                                     font=('Arial', 12))
        self.second_label.place(x=300, y=450, anchor='nw')
        ...
        self.start_button.place(x=380, y=445, anchor='nw')
...
if __name__ == '__main__':
    # 模型初始化
    # config_file = open("config/aishell.yaml")
    # config = AttrDict(yaml.load(config_file, Loader=yaml.FullLoader))
    #
    # model = Transducer(config.model)
    #
    # checkpoint = torch.load(config.training.load_model)  # 现在还没有预训练的模型
