...
class JointNet(nn.Module):
    ...
    def forward(self, enc_state, dec_state):
        ...
        # concat_state = concat_state.half()  # 半精度时
...
class Transducer(nn.Module):
    ...
    def beam_search(self, enc_state, lengths, beam_width=5):
        ...
        for t in range(lengths):  # 循环直到第一次结束暂定以当前概率最高的dec_state以最高的为准
            ...
            dec_state = dec_state[:, -1, :]  # 以当前概率最高的token获得dec_state
            logits = self.joint(enc_state[t].view(-1), dec_state.view(-1))
            out = F.softmax(logits, dim=0).detach()
    ...
    def recognize_beam_search(self, inputs, inputs_length, audio_mask=None):
        """
        batch分开，识别关注之前所有的输出
        :param inputs:
        :param inputs_length:
        ...
        results = []
        for batch in range(batch_size):
            decoded_seq = self.beam_search(enc_states[batch], inputs_length[batch], beam_width=5)
            results.append(decoded_seq)
        return results
